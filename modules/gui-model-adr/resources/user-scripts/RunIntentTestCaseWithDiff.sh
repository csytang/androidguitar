...
## test GUI TO expected output to check ripper### Only works for test apps
echo "Current location"
pwd 
##################DIFF 	Demo.GUI##############################
#if diff "$gui_file" "./testcase-generator-intents/expectedOutput/$1/Demo/Demo.GUI"; then
#	echo "Android ripper produced expected result: file match @$1 Demo.GUI"

#else

#	echo "ERROR: ANDROID RIPPER IS NOT PRODUCING EXPECTED OUTPUT !! FOR ($1 Demo.GUI)"
	#exit 1
#fi
...
wc -w -l < $gui_file > testout1.txt
wc -w -l < ./testcase-generator-intents/expectedOutput/$1/Demo/Demo.GUI > testout2.txt

if diff "testout1.txt" "testout2.txt"; then
	echo "GUI files are a match @$1 Demo.GUI"

	
else

	echo "ERROR: GUI FILE IS NOT A MATCH!! FOR ($1 Demo.GUI)"
	#exit 1
fi
...
# clean up folder so java will not appends to sequence.txt
rm ../../../IntentOutputs/sequence.txt
...
wc -c -l < ../../../IntentOutputs/out.txt > testout1.txt
wc -c -l < testcase-generator-intents/expectedOutput/$1/out.txt > testout2.txt

if diff "testout1.txt" "testout2.txt"; then
	echo "src code parsing match @$1 out.txt"

	
else

	echo "ERROR: PARSING OF SRC CODE IS NOT A MATCH!! FOR ($1 out.txt)"
	exit 1
fi
...
#if diff "../../../IntentOutputs/out.txt" "testcase-generator-intents/expectedOutput/$1/out.txt" "-b"; then
#	echo "src code parsing match @$1 out.txt"
...
##################DIFF 	parsedManifest.txt##############################
if diff "../../../IntentOutputs/parsedManifest.txt" "testcase-generator-intents/expectedOutput/$1/parsedManifest.txt" "-b"; then
	echo "src code parsing match @$1 parsedManifest.txt"

else

	echo "ERROR: PARSING OF AdroidMnifest.xml IS NOT A MATCH!! FOR ($1 parsedManifest.txt)"
	exit 1
fi
##################DIFF sequence.txt##############################
if diff "../../../IntentOutputs/sequence.txt" "testcase-generator-intents/expectedOutput/$1/sequence.txt"; then
	echo "src code parsing match @$1 sequence.txt"

else

	echo "ERROR: tstGenerator IS NOT PRODUCING MATCH!! FOR ($1 sequence.txt)"
	exit 1
fi
...
wc -w -l < ../Demo/testcases/testcase_1.tst > testout1.txt
wc -w -l < ../testcase-generator-intents/expectedOutput/$1/Demo/testcases/testcase_1.tst > testout2.txt

if diff "testout1.txt" "testout2.txt"; then
	echo "testcase_1.tst is a match @$1 testcase_1.tst"

	
else

	echo "ERROR: TESTCASE_1.TST IS NOT A MATCH!! FOR ($1 testcase_1.tst)"
	exit 1
fi
...
str_num_test=$(ls -l ../Demo/testcases | wc -l)
int_num_test=$(expr $str_num_test - 1)
x=1

while [ $x -ne $int_num_test ]
do


	str_char_out1=$(wc -w < ../Demo/testcases/test$x.txt)
	int_char_out1=$(expr $str_char_out1)

	str_line_out1=$(wc -l < ../Demo/testcases/test$x.txt)
	int_line_out1=$(expr $str_line_out1)
	
	str_char_out2=$(wc -w < ../testcase-generator-intents/expectedOutput/ToDoManager/Demo/testcases/test$x.txt)
	int_char_out2=$(expr $str_char_out2)
	
	str_line_out2=$(wc -l < ../testcase-generator-intents/expectedOutput/ToDoManager/Demo/testcases/test$x.txt)
	int_line_out2=$(expr $str_line_out2)

if [ "$int_char_out1" -ne "$int_char_out2" ] || [ "$int_line_out1" -ne "$int_line_out2" ];
then
	echo "failed on test$x"
else
	echo "passed on test$x"
fi
